/**
 * A deliberately tiny semver subset — enough for the `sdk` field of
 * `kiri-plugin.json`, with no dependency.
 *
 * Supported ranges: `*` / `x` / empty (any), an exact version, `=`, `>`, `>=`,
 * `<`, `<=`, `^` and `~`, joined by spaces/commas (AND) and `||` (OR).
 *
...
function satisfiesSet(version: ParsedVersion, comparators: Comparator[]): boolean {
  ...
  if (version.prerelease.length > 0) {
    // Only allow a prerelease when the range explicitly mentions one on the
    // same tuple — otherwise `^2.0.0` would accept `2.1.0-beta.1`.
